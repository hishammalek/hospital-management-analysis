-- =====================================================
 ...
 --
 -- Project Goal:
 -- Analyze patient stay patterns, hospital movement, and operational efficiency
 -- after admission.
 --
 -- Analysis Goal:
...
SELECT
    COUNT(admission_id) AS total_bills,
    ...
    ) AS total_insurance_cover,

    ROUND(
        AVG(insurance_cover), 2
    ) AS average_insurance_cover,

    ROUND(
        SUM(final_amount_payable), 2
    ) AS total_final_amount_payable,

    ROUND(
        AVG(final_amount_payable), 2
    ) AS average_final_amount_payable
...
-- =====================================================
 ...
 --
 -- Business Question:
 -- What proportion of hospital bills are paid versus pending?
 --
 -- Purpose:
...
-- =====================================================
 ...
 --
 -- Business Question:
 -- How much does insurance contirbute to the hospital's total billing amount?
 --
 -- Purpose:
...
SELECT
    ...
    ROUND(
		SUM(insurance_cover) * 100.0 / SUM(total_bill), 2
	) AS percentage_insurance_contribution
FROM billing;
...
SELECT
    ...
    ROUND(
        SUM(insurance_cover) / SUM(total_bill) * 100.0, 2
    ) AS percentage_patients_insurance_cover
FROM billing
GROUP BY admission_id
ORDER BY percentage_patients_insurance_cover DESC;
...
SELECT
    ...
    ) AS percentage_of_total_bills
FROM insurance_group
GROUP BY insurance_percentage_group
ORDER BY insurance_percentage_group;
...
SELECT
    ...
    ROUND(
        SUM(b.insurance_cover), 2
    ) AS total_insurance_cover,

    ROUND(
        SUM(b.insurance_cover) / SUM(b.total_bill) * 100.0, 2
...
ORDER BY total_insurance_cover DESC;
