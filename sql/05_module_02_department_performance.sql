-- =====================================================
 ...
 -- to understand hospital workload, department performance, treatment activity, and revenue patterns.
 --
 -- Analysis Goal:
 -- Identify department workload, activity patterns, cost performance, and revenue contribution
...
SELECT
	...
	) AS department_admissions_percentage
FROM admissions a
JOIN doctors d
...
GROUP BY dp.department_id, dp.department_name, c.year
ORDER BY dp.department_id, admission_year;
...
JOIN calendar c
    ON a.admission_date = c.calendar_date
GROUP BY dp.department_id, dp.department_name, c.year, c.month_no, c.month
ORDER BY dp.department_id, admission_year, admission_month_no;
...
SELECT
    ...
    dp.department_name,
    CASE
        WHEN EXTRACT(DOW FROM a.admission_date) IN (0, 6)
            THEN 'Weekend'
        ELSE 'Weekday'
...
JOIN departments dp
    ON d.department_id = dp.department_id
GROUP BY dp.department_id, dp.department_name, admission_period
ORDER BY dp.department_id, total_admissions DESC;
...
WITH daily_admissions AS (
    ...
    JOIN departments dp
        ON d.department_id = dp.department_id
    GROUP BY dp.department_id, dp.department_name, admission_day
),
...
-- =====================================================
 ...
 --
 -- Business Question:
 -- How are admissions distributed across weekdays for each department?
 --
 -- Purpose:
 -- Identify department-specific peak admission days to support weekly staffing schedules,
 -- operational preparation, and resource allocation.
...
JOIN departments dp
    ON d.department_id = dp.department_id
GROUP BY dp.department_id, dp.department_name, t.treatment_type
ORDER BY dp.department_id, total_treatments DESC;



-- =====================================================
 -- Analysis 2.4B: Treatment Cost by Department
 --
 -- Business Question:
...
SELECT
    ...
    SUM(
        COALESCE(t.treatment_cost, 0)
        + COALESCE(t.medicine_cost, 0)
        + COALESCE(t.lab_cost, 0)
