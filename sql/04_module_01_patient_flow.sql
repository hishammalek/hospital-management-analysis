...
--
-- Business Question:
-- What is the average number of admissions per admitted patient?
--
-- Purpose:
...
SELECT
    ROUND(
        COUNT(a.admission_id) * 1.0 / COUNT(DISTINCT p.patient_id),
        2
...
-- =====================================================
-- Analysis 1.1D: Patient Admission Rate
--
-- Business Question:
-- What percentage of registered patients have been admitted to the hospital?
--
-- Purpose:
-- Measure hospital utilization by identifying the proportion
-- of registered patients who have experienced at least one
-- hospital admission.
-- =====================================================
SELECT
    ...
    ROUND(
        COUNT(DISTINCT a.patient_id) * 100.0 / COUNT(DISTINCT p.patient_id),
        2
...
-- =====================================================
-- Analysis 1.1E: Repeat-Admitted Patient Rate
--
-- Business Question:
-- What percentage of admitted patients have experienced multiple
-- hospital admissions?
--
-- Purpose:
...
--
-- Purpose:
-- Identify patients with unusually high admission frequency patterns 
-- to understand repeat utilization patterns and potential demand
-- concentration.
-- =====================================================
WITH
    patient_summary AS (
...
GROUP BY
    ...
    c.month
ORDER BY
    monthly_admissions DESC
LIMIT
    5;

-- =====================================================
-- Analysis 1.3: Room Type Distribution & Utilization
--
-- Business Question:
-- How are hospital admissions distributed across different
-- room types?
--
-- Purpose:
-- Analyze admission volume and utilization share across General,
-- Private, and ICU rooms to understand hospital capacity demand 
-- and resource allocation.
-- =====================================================
SELECT
    room_type,
    COUNT(*) AS room_type_admissions,
    ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (), 2) AS room_type_percentage
FROM
    admissions
GROUP BY
    room_type
ORDER BY
    room_type_admissions DESC;

-- =====================================================
-- Analysis 1.4A: Admission Pattern by Day
--
-- Business Question:
-- How is hospital admission activity distributed across the days
-- of the week?
--
-- Purpose:
...
SELECT
    ...
    COUNT(*) AS total_admissions,
    ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (), 2) AS admissions_percentage
FROM
    admissions
GROUP BY
    admission_period
...
-- =====================================================
-- Analysis 1.5: Yearly Admission Trend & YoY Growth
--
-- Business Question:
-- How has hospital admission volume changed from year to year
-- across 2020-2024?
--
-- Purpose:
-- Analyze annual admission volume and year-over-year growth to identify
-- changes in hospital demand and support long-term capacity and
-- resource planning.
-- =====================================================
WITH
...
SELECT
    ...
    ROUND(
        (total_admissions - previous_year_admissions) * 100.0 / previous_year_admissions,
        2
