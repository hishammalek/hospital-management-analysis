...
--
-- Analysis Goal:
-- Evaluate doctor workload, patient volume, clinical activity, resource utilization, and
-- billing patterns to support staffing, capacity planning, and hospital resource allocation
-- decisions.
-- =====================================================
...
SELECT
	...
	department_name,
	total_admissions,
	ROUND(AVG(total_admissions) OVER (), 2) AS average_doctor_admissions,
	ROUND(
	...
	ROUND(
		(
			(total_admissions - AVG(total_admissions) OVER ()) / AVG(total_admissions) OVER () * 100
		),
		2
	) AS workload_percentage_difference
FROM
	admissions_patients_data
ORDER BY
	workload_percentage_difference DESC
LIMIT
	10;
...
WITH
	...
	ranked_doctors AS (
		SELECT
			...
			treatments,
			ROUND(
				treatments * 100.0 / SUM(treatments) OVER (
					PARTITION BY
						doctor_id
				),
				2
...
FROM
	ranked_doctors
WHERE
	treatment_type_rank = 1
ORDER BY
	treatment_percentage DESC;
...
SELECT
	...
	SUM(
		CASE
			WHEN room_type = 'General' THEN total_admissions
			ELSE 0
		END
	) AS general_admissions,
	SUM(
		CASE
			WHEN room_type = 'Private' THEN total_admissions
			ELSE 0
		END
	) AS private_admissions,
	SUM(
		CASE
			WHEN room_type = 'ICU' THEN total_admissions
			ELSE 0
		END
	) AS icu_admissions
FROM
...
GROUP BY
	...
	department_name
ORDER BY
	general_admissions DESC;

-- =====================================================
-- Analysis 5.2C: Average Length of Stay by Doctor
--
-- Business Question:
...
-- =====================================================
-- Analysis 5.2D: Treatment Type Diversity by Doctor
--
-- Business Question:
-- How diverse are the treatment types handled by each doctor?
--
-- Purpose:
-- Analyze the breadth of treatment types handled by each doctor
-- to understand clinical workload diversity and identify doctors
-- with broader or more specialized treatment responsibilities.
-- =====================================================
SELECT
	d.doctor_id,
	d.doctor_name,
	...
	COUNT(t.admission_id) AS total_treatments
FROM
	treatment t
	JOIN admissions a ON t.admission_id = a.admission_id
	JOIN doctors d ON a.doctor_id = d.doctor_id
...
SELECT
	...
	dp.department_name,
	SUM(b.total_bill) AS total_revenue,
	ROUND(AVG(b.total_bill), 2) AS average_bill_per_admission
FROM
	billing b
	JOIN admissions a ON b.admission_id = a.admission_id
	JOIN doctors d ON a.doctor_id = d.doctor_id
...
GROUP BY
	...
	dp.department_name
ORDER BY
	average_bill_per_admission DESC
LIMIT
	10;
